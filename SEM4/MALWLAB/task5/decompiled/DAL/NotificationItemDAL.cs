...
namespace SolarWinds.Orion.Core.BusinessLayer.DAL
{
  public class NotificationItemDAL
  {
    ...
    public bool IsAcknowledged => this.AcknowledgedAt.HasValue;

    public bool IsVisible => !this.IsAcknowledged && !this.Ignored;
    ...
    protected virtual Guid GetNotificationItemTypeId() => Guid.Empty;
    ...
    public static TNotificationItem GetItemById<TNotificationItem>(Guid itemId) where TNotificationItem : NotificationItemDAL, new()
    {
      try
      {
        TNotificationItem itemById = new TNotificationItem();
        itemById.Id = itemId;
        itemById.LoadFromDB();
        return itemById;
      }
      ...
      NotificationItemFilter filter)
    {
      ...
      try
      {
        using (SqlCommand sqlCommand = new NotificationItemDAL().ComposeSelectCollectionCommand(typeId, filter))
        {
          ...
          {
            List<NotificationItemDAL> itemsByTypeId = new List<NotificationItemDAL>();
            while (rd.Read())
            {
              NotificationItemDAL notificationItemDal = new NotificationItemDAL();
              notificationItemDal.LoadFromReader(rd);
              itemsByTypeId.Add(notificationItemDal);
            }
            return (ICollection<NotificationItemDAL>) itemsByTypeId;
          }
        }
      }
      catch (ResultCountException ex)
    ...
    {
      TNotificationItem latestItem = new TNotificationItem();
      using (SqlCommand sqlCommand = latestItem.ComposeSelectLatestItemCommand(filter))
      {
        using (IDataReader rd = SqlHelper.ExecuteReader(sqlCommand))
        {
          if (!rd.Read())
            return default (TNotificationItem);
          latestItem.LoadFromReader(rd);
          return latestItem;
        }
      }
    }
    ...
    public static NotificationItemDAL GetLatestItemByType(
      ...
      NotificationItemFilter filter)
    {
      NotificationItemDAL latestItemByType = new NotificationItemDAL();
      using (SqlCommand sqlCommand = latestItemByType.ComposeSelectLatestItemCommand(typeId, filter))
      {
        using (IDataReader rd = SqlHelper.ExecuteReader(sqlCommand))
        {
          if (!rd.Read())
            return (NotificationItemDAL) null;
          latestItemByType.LoadFromReader(rd);
          return latestItemByType;
        }
      }
    }
    ...
    {
      this.Id = rd != null ? DatabaseFunctions.GetGuid(rd, "NotificationID") : throw new ArgumentNullException(nameof (rd));
    ...
    {
      Dictionary<Guid, int> notificationsCounts = new Dictionary<Guid, int>();
      using (SqlCommand textCommand = SqlHelper.GetTextCommand("SELECT NotificationTypeID, COUNT(NotificationID) as TheCount FROM NotificationItems WHERE AcknowledgedAt IS NULL AND Ignored=0 GROUP BY NotificationTypeID"))
      {
        using (IDataReader dataReader = SqlHelper.ExecuteReader(textCommand))
        {
          while (dataReader.Read())
          {
            ...
            notificationsCounts[guid] = int32;
          }
        }
      }
