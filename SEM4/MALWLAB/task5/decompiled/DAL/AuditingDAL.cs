...
namespace SolarWinds.Orion.Core.BusinessLayer.DAL
{
  public class AuditingDAL
  {
    ...
    public List<AuditActionTypeInfo> GetAuditingActionTypes()
    {
      lock (this.locker)
      {
        if (this.actionTypes.Count == 0)
          this.LoadKeys();
        return this.actionTypes.Select<KeyValuePair<AuditActionType, int>, AuditActionTypeInfo>((System.Func<KeyValuePair<AuditActionType, int>, AuditActionTypeInfo>) (i => new AuditActionTypeInfo()
        {
    ...
    protected int GetActionIdFromActionType(AuditActionType actionType)
    {
      ...
      int idFromActionType = -1;
      lock (this.locker)
      {
        if (this.actionTypes.TryGetValue(actionType, out idFromActionType))
          return idFromActionType;
      }
      if (this.LoadKeys())
      {
        lock (this.locker)
    ...
    public AuditActionType GetActionTypeFromActionId(int actionTypeId)
    {
      ...
      lock (this.locker)
      {
        using (IEnumerator<KeyValuePair<AuditActionType, int>> enumerator = this.actionTypes.Where<KeyValuePair<AuditActionType, int>>((System.Func<KeyValuePair<AuditActionType, int>, bool>) (actionType => actionType.Value == actionTypeId)).GetEnumerator())
        {
    ...
    {
      return container != null ? this.StoreNotification(container, this.GetActionIdFromActionType(container.ActionType)) : throw new ArgumentNullException(nameof (container));
    }
    ...
    private AuditingDAL.NetObjectInfo GetNetObjectInfo(IDictionary<string, string> arguments)
    {
      ...
      {
        string[] netObject = NetObjectHelper.ParseNetObject(arguments[KnownKeys.NetObject]);
        str = netObject[0];
        nullable2 = new int?(int.Parse(netObject[1]));
      }
      if (arguments.ContainsKey(KnownKeys.NodeID))
        nullable1 = new int?(int.Parse(arguments[KnownKeys.NodeID]));
      ...
      int actionTypeId)
    {
      ...
      AuditingDAL.NetObjectInfo netObjectInfo = this.GetNetObjectInfo((IDictionary<string, string>) decoratedDecoratedContainer.Args);
      ...
      if (count > 0)
      {
        ...
        stringBuilder.Append(string.Join(" UNION ALL ", Enumerable.Range(0, count).Select<int, string>((System.Func<int, string>) (i => string.Format(" SELECT @lastID, @ArgsKey{0}, @ArgsValue{0} ", (object) i)))));
      }
        ...
        using (SqlTransaction sqlTransaction = connection.BeginTransaction())
        {
          using (SqlCommand textCommand = SqlHelper.GetTextCommand(stringBuilder.ToString()))
          {
            try
            {
              ...
              {
                string key = decoratedDecoratedContainer.Args.ElementAt<KeyValuePair<string, string>>(index).Key;
                string str = decoratedDecoratedContainer.Args.ElementAt<KeyValuePair<string, string>>(index).Value;
                if (AuditingDAL.log.IsDebugEnabled)
      ...
      DateTime endTime)
    {
      ...
      using (SqlCommand textCommand = SqlHelper.GetTextCommand(string.Empty))
      {
        ...
        if (!string.IsNullOrEmpty(actionTypeIds))
          values.Add(" AE.ActionTypeID IN (" + actionTypeIds + ")");
        string str1 = string.Join(" AND " + Environment.NewLine, (IEnumerable<string>) values);
        string str2 = string.Format(format, (object) str1);
        textCommand.CommandText = str2;
        DataTable auditingTable = SqlHelper.ExecuteDataTable(textCommand);
        auditingTable.TableName = "AuditingTable";
    ...
    public static DataTable GetAuditingTypesTable()
    {
      ...
      {
        DataTable auditingTypesTable = InformationServiceProxyExtensions.QueryWithAppendedErrors((IInformationServiceProxy) iinformationServiceProxy2, "SELECT DISTINCT ActionTypeID, ActionType, ActionTypeDisplayName FROM [Orion].[AuditingActionTypes] (nolock=true)", SwisFederationInfo.IsFederationEnabled);
        auditingTypesTable.TableName = "AuditingTypesTable";
        return auditingTypesTable;
      }
    }
