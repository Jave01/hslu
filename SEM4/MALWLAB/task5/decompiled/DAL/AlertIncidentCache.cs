...
namespace SolarWinds.Orion.Core.BusinessLayer.DAL
{
  internal class AlertIncidentCache
  {
    ...
    public void FillIncidentInfo(ActiveAlert activeAlert)
    {
      ...
      if (!this.incidentInfoByAlertObjectId.TryGetValue(activeAlert.Id, out source) || source.Count == 0)
        return;
      if (source.Count == 1)
      {
        activeAlert.IncidentNumber = source[0].Number;
        activeAlert.IncidentUrl = source[0].Url;
        activeAlert.AssignedTo = source[0].AssignedTo;
      }
      else
      {
        activeAlert.IncidentNumber = string.Format((IFormatProvider) CultureInfo.InvariantCulture, Resources2.ActiveAlertsGrid_IncidentsClomun_ValueFormat, (object) source.Count);
        activeAlert.IncidentUrl = string.Format((IFormatProvider) CultureInfo.InvariantCulture, "/Orion/View.aspx?NetObject=AAT:{0}", (object) activeAlert.Id);
        List<string> list = source.Select<AlertIncidentCache.IncidentInfo, string>((System.Func<AlertIncidentCache.IncidentInfo, string>) (i => i.AssignedTo)).Where<string>((System.Func<string, bool>) (u => !string.IsNullOrEmpty(u))).Distinct<string>().ToList<string>();
        if (list.Count == 1)
          activeAlert.AssignedTo = list.First<string>();
        else
