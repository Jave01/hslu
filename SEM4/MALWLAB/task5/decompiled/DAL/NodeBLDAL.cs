...
namespace SolarWinds.Orion.Core.BusinessLayer.DAL
{
  public class NodeBLDAL : INodeBLDAL
  {
    ...
    public static Nodes GetNodesWithIPs(string[] addresses)
    {
      ...
      Nodes nodesWithIps = Collection<int, SolarWinds.Orion.Core.Common.Models.Node>.FillCollection<Nodes, bool>(new Collection<int, SolarWinds.Orion.Core.Common.Models.Node>.CreateElementWithParams<bool>((object) null, __methodptr(CreateNode)), stringBuilder.ToString(), (SqlParameter[]) null, new bool[2]);
      if (((Collection<int, SolarWinds.Orion.Core.Common.Models.Node>) nodesWithIps).Count > 0)
      {
        foreach (Interface nodesInterface in (Collection<int, Interface>) InterfaceBLDAL.GetNodesInterfaces(nodesWithIps.NodesIDs))
          ((Collection<int, Interface>) ((Collection<int, SolarWinds.Orion.Core.Common.Models.Node>) nodesWithIps).FindByID(nodesInterface.NodeID)?.Interfaces).Add((object) nodesInterface);
        foreach (Volume nodesVolume in (Collection<int, Volume>) VolumeDAL.GetNodesVolumes(nodesWithIps.NodesIDs))
          ((Collection<int, Volume>) ((Collection<int, SolarWinds.Orion.Core.Common.Models.Node>) nodesWithIps).FindByID(nodesVolume.NodeID)?.Volumes).Add((object) nodesVolume);
      }
      return nodesWithIps;
    }

    public static Nodes GetNodes() => NodeBLDAL.GetNodes(true, true);

    public static Nodes GetNodes(bool includeInterfaces, bool includeVolumes)
    {
      // ISSUE: method pointer
      Nodes nodes = Collection<int, SolarWinds.Orion.Core.Common.Models.Node>.FillCollection<Nodes, bool>(new Collection<int, SolarWinds.Orion.Core.Common.Models.Node>.CreateElementWithParams<bool>((object) null, __methodptr(CreateNode)), "SELECT * FROM dbo.Nodes", (SqlParameter[]) null, new bool[2]);
      if (includeInterfaces && NodeBLDAL._areInterfacesAllowed)
      {
        foreach (Interface @interface in (Collection<int, Interface>) InterfaceBLDAL.GetInterfaces())
          ((Collection<int, Interface>) ((Collection<int, SolarWinds.Orion.Core.Common.Models.Node>) nodes).FindByID(@interface.NodeID)?.Interfaces).Add((object) @interface);
      }
      if (includeVolumes)
      {
        foreach (Volume volume in (Collection<int, Volume>) VolumeDAL.GetVolumes())
          ((Collection<int, Volume>) ((Collection<int, SolarWinds.Orion.Core.Common.Models.Node>) nodes).FindByID(volume.NodeID)?.Volumes).Add((object) volume);
      }
      return nodes;
    }
    ...
    public static Nodes GetNodesByIds(int[] nodeIds)
    {
      ...
      // ISSUE: method pointer
      return Collection<int, SolarWinds.Orion.Core.Common.Models.Node>.FillCollection<Nodes, bool>(new Collection<int, SolarWinds.Orion.Core.Common.Models.Node>.CreateElementWithParams<bool>((object) null, __methodptr(CreateNode)), string.Format("SELECT * FROM dbo.Nodes WHERE dbo.Nodes.NodeID in ({0})", (object) stringBuilder), (SqlParameter[]) null, new bool[2]);
    }
    ...
    public static List<string> GetFields() => NodeBLDAL._fields;
        ...
        using (IDataReader dataReader = SqlHelper.ExecuteReader(textCommand))
        {
          while (dataReader.Read())
          {
            ...
            if (!vendors.ContainsKey(key))
              vendors.Add(key, DatabaseFunctions.GetString(dataReader, "Vendor"));
          }
        }
      }
      return vendors;
    ...
    {
      // ISSUE: method pointer
      return Collection<int, SolarWinds.Orion.Core.Common.Models.Node>.GetCollectionItem<Nodes, bool>(new Collection<int, SolarWinds.Orion.Core.Common.Models.Node>.CreateElementWithParams<bool>((object) null, __methodptr(CreateNode)), "Select * from dbo.Nodes WHERE Nodes.NodeID=@NodeId", new SqlParameter[1]
      {
      ...
      bool includeVolumes)
    {
      ...
      {
        new SqlParameter("@NodeId", (object) nodeId)
      }, new bool[2]{ includeInterfaces, includeVolumes }) ?? throw new ArgumentOutOfRangeException(nameof (nodeId), string.Format("Node Id {0} does not exist", (object) nodeId));
    }
    ...
    {
      NodeBLDAL.DeleteNode(NodeBLDAL.GetNode(nodeId) ?? throw new ArgumentOutOfRangeException(nameof (nodeId), string.Format("Node Id {0} does not exist", (object) nodeId)));
    ...
    public static void DeleteNode(SolarWinds.Orion.Core.Common.Models.Node node)
    {
      ...
      {
        foreach (Volume volume in (Collection<int, Volume>) node.Volumes)
      ...
      using (SqlCommand textCommand = SqlHelper.GetTextCommand("DELETE FROM Pollers WHERE NetObjectType=@NetObjectType AND NetObjectID=@NetObjectID"))
      {
        ...
        textCommand.Parameters.Add("@NetObjectID", SqlDbType.Int).Value = (object) node.ID;
        SqlHelper.ExecuteNonQuery(textCommand);
      }
      EventsDAL.InsertEvent(node.ID, node.ID, "N", 17, string.Format(Resources.COREBLCODE_IB0_1, (object) node.Caption));
    }
    ...
    public static NodeHardwareType GetNodeHardwareType(int nodeId) => (NodeHardwareType) 0;
    ...
    public static List<int> EnumerateUnmanagedNodes() => NodeDAL.EnumerateUnmanagedNodes();
      ...
      Dictionary<string, object> filters)
    {
      ...
      Dictionary<string, int> propertyFiltered = new Dictionary<string, int>();
        ...
        using (IDataReader dataReader = SqlHelper.ExecuteReader(textCommand))
        {
          while (dataReader.Read())
          {
            if (dataReader[2] is bool)
              propertyFiltered.Add(dataReader[2].ToString(), DatabaseFunctions.GetInt32(dataReader, 1));
            else if (dataReader[2] is int)
              propertyFiltered.Add(dataReader[3].ToString(), DatabaseFunctions.GetInt32(dataReader, 1));
            else if (dataReader[2] is float)
            {
              if (string.IsNullOrEmpty(dataReader[3].ToString()))
                propertyFiltered.Add(dataReader[3].ToString(), DatabaseFunctions.GetInt32(dataReader, 1));
              else
                propertyFiltered.Add(DatabaseFunctions.GetFloat(dataReader, 2).ToString((IFormatProvider) CultureInfo.InvariantCulture.NumberFormat), DatabaseFunctions.GetInt32(dataReader, 1));
            }
            else if (dataReader[2] is DateTime)
            {
              if (dataReader[3].GetType() == typeof (DBNull))
                ...
                propertyFiltered.Add(dataReader[0].ToString(), DatabaseFunctions.GetInt32(dataReader, 1));
            }
            else
              propertyFiltered.Add(dataReader[0].ToString(), DatabaseFunctions.GetInt32(dataReader, 1));
          }
    ...
    internal static Dictionary<string, int> GetValuesAndCountsForProperty(
      ...
      CultureInfo culture)
    {
      Dictionary<string, int> countsForProperty = new Dictionary<string, int>();
        ...
        using (IDataReader dataReader = SqlHelper.ExecuteReader(textCommand))
        {
          while (dataReader.Read())
          {
            if (dataReader[0].GetType() == typeof (DateTime))
              countsForProperty.Add(dataReader.GetDateTime(0).ToString((IFormatProvider) culture), DatabaseFunctions.GetInt32(dataReader, 1));
            else if (dataReader[0] is float)
            {
              if (string.IsNullOrEmpty(dataReader[2].ToString()))
                countsForProperty.Add(dataReader[2].ToString(), DatabaseFunctions.GetInt32(dataReader, 1));
              else
                countsForProperty.Add(dataReader.GetFloat(0).ToString((IFormatProvider) culture.NumberFormat), DatabaseFunctions.GetInt32(dataReader, 1));
            }
            else if (dataReader[0] is int || dataReader[0] is byte)
            {
              if (string.IsNullOrEmpty(dataReader[2].ToString()))
                ...
                countsForProperty.Add(dataReader[0].ToString(), DatabaseFunctions.GetInt32(dataReader, 1));
            }
            else
              countsForProperty.Add(dataReader[0].ToString(), DatabaseFunctions.GetInt32(dataReader, 1));
          }
...
label_14:
      ...
      // ISSUE: method pointer
      Nodes nodesFiltered = Collection<int, SolarWinds.Orion.Core.Common.Models.Node>.FillCollection<Nodes, bool>(new Collection<int, SolarWinds.Orion.Core.Common.Models.Node>.CreateElementWithParams<bool>((object) null, __methodptr(CreateNode)), "SELECT * from dbo.Nodes " + str2 + " ORDER BY Caption", sqlParameterList.ToArray(), new bool[2]);
      if (includeInterfaces && NodeBLDAL._areInterfacesAllowed)
      {
        using (SqlCommand textCommand = SqlHelper.GetTextCommand("SELECT Interfaces.* FROM Interfaces \r\nINNER JOIN Nodes on Interfaces.NodeID=Nodes.NodeID" + str2 + " ORDER BY Interfaces.Caption"))
        {
          ...
          using (IDataReader reader = SqlHelper.ExecuteReader(textCommand))
          {
            while (reader.Read())
            {
              ...
              ((Collection<int, Interface>) ((Collection<int, SolarWinds.Orion.Core.Common.Models.Node>) nodesFiltered).FindByID(@interface.NodeID).Interfaces).Add((object) @interface);
            }
          }
        }
      }
        ...
        using (SqlCommand textCommand = SqlHelper.GetTextCommand("SELECT Volumes.* FROM Volumes \r\nINNER JOIN Nodes on Volumes.NodeID=Nodes.NodeID" + str2 + " ORDER BY Volumes.Caption"))
        {
          ...
          using (IDataReader reader = SqlHelper.ExecuteReader(textCommand))
          {
            while (reader.Read())
            {
              ...
              ((Collection<int, Volume>) ((Collection<int, SolarWinds.Orion.Core.Common.Models.Node>) nodesFiltered).FindByID(volume.NodeID).Volumes).Add((object) volume);
            }
          }
        }
      }
    ...
    {
      Dictionary<string, string> vendorIconFileNames = new Dictionary<string, string>();
      using (SqlCommand textCommand = SqlHelper.GetTextCommand("SELECT DISTINCT Vendor, VendorIcon FROM Nodes WITH (NOLOCK)"))
      {
        using (IDataReader dataReader = SqlHelper.ExecuteReader(textCommand))
        {
          while (dataReader.Read())
            vendorIconFileNames[DatabaseFunctions.GetString(dataReader, 0)] = DatabaseFunctions.GetString(dataReader, 1).Trim();
        }
      }
      return vendorIconFileNames;
    }
      ...
      ICollection<string> properties)
    {
      ...
      using (macroParser.MyDBConnection = DatabaseFunctions.CreateConnection())
      {
        foreach (string property in (IEnumerable<string>) properties)
        {
          ...
          {
            object customProperty = node.CustomProperties[key];
      ...
      string searchText)
    {
      ...
      bool flag = SolarWinds.Orion.Core.Common.EntityManager.EntityManager.InstanceWithCache.IsThereEntity("Orion.NPM.EW.Entity");
      DataTable pagebleNodes;
      using (SqlCommand textCommand = SqlHelper.GetTextCommand(string.Format("\r\nSelect [NodeID], [IP_Address], [Caption], [Status], [StatusLED]\r\nfrom (SELECT N.*, ROW_NUMBER() OVER (ORDER BY {0} {1}) RowNr from [Nodes] N {4} WHERE {5}) t\r\nWHERE RowNr BETWEEN {2} AND {3} \r\nORDER BY {0} {1}", string.IsNullOrEmpty(column) ? (object) "Caption" : (object) column, string.IsNullOrEmpty(direction) ? (object) "ASC" : (object) direction, (object) (number + 1), (object) (number + size), flag ? (object) " LEFT JOIN NPM_NV_EW_NODES_V EW ON (EW.NodeID = N.NodeID) " : (object) "", (object) (NodeBLDAL.GetWhere("N", property, proptype, val) + (string.IsNullOrEmpty(searchText) ? string.Empty : string.Format(" And (N.Caption Like '{0}')", (object) searchText))))))
        pagebleNodes = SqlHelper.ExecuteDataTable(textCommand);
      if (pagebleNodes != null)
        pagebleNodes.TableName = "PagebleNodes";
      return pagebleNodes;
    }
    ...
    public static DataTable GetGroupsByNodeProperty(string property, string propertyType)
    {
      ...
      DataTable groupsByNodeProperty = new DataTable();
      using (SqlCommand textCommand = SqlHelper.GetTextCommand(str))
        groupsByNodeProperty = SqlHelper.ExecuteDataTable(textCommand);
      if (groupsByNodeProperty != null)
        groupsByNodeProperty.TableName = "GroupsByNodeProperty";
      return groupsByNodeProperty;
    }
    ...
    public static string GetWhere(string tableAlias, string prop, string type, string value)
    {
      ...
      if (prop.Equals("MachineType", StringComparison.OrdinalIgnoreCase) && (value.Equals("null", StringComparison.OrdinalIgnoreCase) || string.IsNullOrEmpty(value.Trim())))
        return string.Format(" ({0}.{1} IS NULL OR {0}.{1}='' OR {0}.{1}='{2}') ", (object) tableAlias, (object) prop, (object) Resources.COREBUSINESSLAYERDAL_CODE_YK0_8);
    ...
    public static DataTable GetNodeCPUsByPercentLoad(int nodeId, int pageNumber, int pageSize)
    {
      ...
      using (SqlCommand textCommand = SqlHelper.GetTextCommand(string.Format("declare @@cpuCount int\r\n\r\nSelect @@cpuCount = count(Distinct CPUIndex)  FROM CPUMultiLoad\r\nWHERE NodeID = @NodeId\r\n\r\nif @@cpuCount <= 1\r\n Select top 0 CPUIndex AS CPUName, CPUIndex, AvgLoad, TimeStampUTC from CPUMultiLoad\r\nelse\r\nSelect * from\r\n(\r\nSelect ROW_NUMBER() OVER (order by z.AvgLoad DESC) as rnbr, * from\r\n(\r\nSELECT N'{0}' + CAST(CPUIndex + 1 as varchar(6)) as CPUName, CPUIndex, AvgLoad, TimeStampUTC,\r\n DENSE_RANK() OVER (PARTITION BY CPUIndex ORDER BY TimeStampUTC desc) AS Rank\r\n FROM CPUMultiLoad\r\nWHERE NodeID = @NodeId) as z\r\nwhere z.Rank = 1\r\n) t1 where t1.rnbr >((@PageNumber-1)*@PageSize) and t1.rnbr <=((@PageNumber)*@PageSize)\r\nORDER BY AvgLoad DESC\r\n", (object) string.Format(Resources.LIBCODE_IB0_1, (object) string.Empty))))
      {
        ...
        cpUsByPercentLoad = SqlHelper.ExecuteDataTable(textCommand);
      }
      if (cpUsByPercentLoad != null)
        cpUsByPercentLoad.TableName = "NodeCPUsByPercentLoad";
      return cpUsByPercentLoad;
    }
    ...
    public static DataTable GetNodesCpuIndexCounts(List<string> nodeIds)
    {
      ...
      using (SqlCommand textCommand = SqlHelper.GetTextCommand(string.Format("SELECT DISTINCT(z.NodeID) as ID, z.Caption, COUNT(z.CPUIndex) As Cnt from\r\n(SELECT cpu.NodeID, cpu.CPUIndex, n.Caption, cpu.TimeStampUTC,\r\n DENSE_RANK() OVER (PARTITION BY cpu.NodeID ORDER BY cpu.TimeStampUTC desc) AS [Rank]\r\n FROM CPUMultiLoad cpu\r\n INNER JOIN Nodes n ON cpu.NodeID = n.NodeID\r\nWHERE cpu.NodeID in ({0}) \r\n) as z\r\nwhere z.Rank = 1\r\nGROUP BY z.NodeID, z.Caption\r\nORDER BY z.Caption", (object) string.Join(",", nodeIds.ToArray()))))
        nodesCpuIndexCounts = SqlHelper.ExecuteDataTable(textCommand);
      if (nodesCpuIndexCounts != null)
        nodesCpuIndexCounts.TableName = "NodeCPUsByPercentLoad";
      return nodesCpuIndexCounts;
    }
