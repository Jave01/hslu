...
namespace SolarWinds.Orion.Core.BusinessLayer.Agent
{
  internal class AgentDeployer
  {
    ...
    internal AgentDeployer(IAgentInfoDAL agentInfoDal) => this.agentInfoDal = agentInfoDal;
    ...
    public int StartDeployingAgent(AgentDeploymentSettings settings)
    {
      ...
      instance.AddOrUpdateDeploymentInfo(AgentDeploymentInfo.Calculate(new AgentManager(this.agentInfoDal).GetAgentInfo(agentId), (IEnumerable<string>) settings.RequiredPlugins, (string) null));
      this.DeployMissingPlugins(agentId, (IEnumerable<string>) settings.RequiredPlugins);
      return agentId;
    }
      ...
      Action<AgentDeploymentStatus> onFinishedCallback = null)
    {
      ...
      deploymentInfo.StatusInfo = new AgentDeploymentStatusInfo(agentId, (AgentDeploymentStatus) 0);
      AgentDeploymentWatcher instance = AgentDeploymentWatcher.GetInstance(this.agentInfoDal);
    ...
    private void DeployMissingPlugins(int agentId, IEnumerable<string> requiredPlugins)
    {
      ...
      foreach (string pluginId in agentInfo.Plugins.Where<AgentPluginInfo>((Func<AgentPluginInfo, bool>) (p => p.Status == 5 || p.Status == 12)).Select<AgentPluginInfo, string>((Func<AgentPluginInfo, string>) (p => p.PluginId)).ToArray<string>())
      {
      ...
      foreach (string pluginId in requiredPlugins.Where<string>((Func<string, bool>) (requiredPluginId => agentInfo.Plugins.All<AgentPluginInfo>((Func<AgentPluginInfo, bool>) (installedPlugin => installedPlugin.PluginId != requiredPluginId)))))
      {
      ...
      if (agentInfo == null || agentInfo.AgentStatus != 8 && !agentInfo.Plugins.Any<AgentPluginInfo>((Func<AgentPluginInfo, bool>) (p => p.Status == 3)) && !agentInfo.Plugins.Any<AgentPluginInfo>((Func<AgentPluginInfo, bool>) (p => p.Status == 13)))
        return;
    ...
    private int DeployAgent(AgentDeploymentSettings settings)
    {
      ...
      agentManager.UpdateAgentNodeId(agentId, settings.NodeId);
