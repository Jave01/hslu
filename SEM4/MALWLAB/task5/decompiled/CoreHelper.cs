  ...
  public class CoreHelper
  {
    ...
    private static bool ShowUpdateProductNotification(string productTag)
    {
      ...
      try
      {
        ...
        {
          CoreHelper._log.DebugFormat("Calling SWQL query: {0}", (object) moduleInfo.ValidateUpdateNotification);
          DataTable dataTable = ((IInformationServiceProxy) iinformationServiceProxy2).Query(moduleInfo.ValidateUpdateNotification);
          if (dataTable.Columns.Count == 1 || dataTable.Rows.Count == 1)
            return dataTable.Rows[0][0] == null || Convert.ToBoolean(dataTable.Rows[0][0]);
          CoreHelper._log.WarnFormat("Invalid query: {0}", (object) moduleInfo.ValidateUpdateNotification);
          return true;
        }
      }
      catch (Exception ex)
      {
        CoreHelper._log.ErrorFormat("Execution of ValidateUpdateNotification '{0}' has failed. Exception: {1}", (object) moduleInfo.ValidateUpdateNotification, (object) ex);
        return true;
      }
    }
