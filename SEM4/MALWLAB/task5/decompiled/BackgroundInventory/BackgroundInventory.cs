...
namespace SolarWinds.Orion.Core.BusinessLayer.BackgroundInventory
{
  public class BackgroundInventory : IDisposable
  {
    ...
    public BackgroundInventory(int parallelTasksCount, Dictionary<string, object> plugins)
    {
      ...
      this.wmiGlobals.ConnectionRationMode = SettingsDAL.GetCurrent<WmiConnectionRationMode>("SWNetPerfMon-Settings-Wmi ConnectionRationMode", (WmiConnectionRationMode) 1);
    ...
    public void DoInventory(SolarWinds.Orion.Core.BusinessLayer.BackgroundInventory.BackgroundInventory.InventoryTask task)
    {
      ...
      else
      {
        ...
        string[] source = task.Settings.Split(':');
              ...
              else
              {
                ...
                else
                {
                  ...
                  if (result.Error != null)
                  {
                    ...
                    {
                      List<string> list = ((IEnumerable<string>) source).Where<string>((Func<string, bool>) (n => !completedTasks.Contains(n) && !failedTasks.Contains(n))).ToList<string>();
                      if (list.Count > 0)
                      {
        ...
        else
        {
          ...
          else if (failedTasks.Count < source.Length)
          {
