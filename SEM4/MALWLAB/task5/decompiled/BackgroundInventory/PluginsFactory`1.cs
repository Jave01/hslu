...
namespace SolarWinds.Orion.Core.BusinessLayer.BackgroundInventory
{
  internal sealed class PluginsFactory<T>
  {
      ...
      ResolveEventArgs args)
    {
      ...
      AggregateException aggregateException = new AggregateException(new StringBuilder("inspecting [").Append(args.Name).Append(']').Append(PluginsFactory<T>.FormatRequest(requestingAssembly)).Append('.').ToString(), (IEnumerable<Exception>) innerExceptions);
