...
namespace SolarWinds.Orion.Core.BusinessLayer
{
  public class UpdateTaskScheduler<TTaskKey, TCallbackArg> : IDisposable where TTaskKey : IComparable
  {
      ...
      TimeSpan mandatorySchedulerDelay)
    {
      ...
      stpStartInfo.MaxWorkerThreads = maxRunningTasks;
      stpStartInfo.MinWorkerThreads = 0;
      ((WIGStartInfo) stpStartInfo).StartSuspended = false;
      ...
      TimeSpan waitForChangesDelay)
    {
      ...
      lock (this.syncLock)
      {
        UpdateTaskScheduler<TTaskKey, TCallbackArg>.ScheduledTask scheduledTask3 = (UpdateTaskScheduler<TTaskKey, TCallbackArg>.ScheduledTask) null;
    ...
    private void SchedulerRoutine()
    {
      ...
      while (true)
      {
        ...
        {
          this.processingGroup.QueueWorkItem(new WorkItemCallback(this.ThreadPoolRoutine), (object) scheduledTask);
          if (this.log.IsDebugEnabled)
        ...
        using (LocaleThreadState.EnsurePrimaryLocale())
        {
          ...
          if (scheduledTask.Callbacks != null)
          {
            foreach (UpdateTaskScheduler<TTaskKey, TCallbackArg>.ScheduledTaskCallback callback in scheduledTask.Callbacks)
            {
              ...
              {
                this.log.Error((object) string.Format("Task {0} callback failed", (object) scheduledTask), ex3);
              }
            }
          }
        }
    ...
    ~UpdateTaskScheduler() => this.Dispose(false);
    ...
    public class ScheduledTaskCallbackEventArgs : EventArgs
    {
      ...
      public bool TaskFailed => this.TaskException != null;
    ...
    private class ScheduledTask
    {
      ...
      public override string ToString() => this.TaskKey.ToString();

      public class IdentityComparer : 
        Comparer<UpdateTaskScheduler<TTaskKey, TCallbackArg>.ScheduledTask>,
        IEqualityComparer<UpdateTaskScheduler<TTaskKey, TCallbackArg>.ScheduledTask>
      {
      ...
      public class SchedulingComparer : 
        Comparer<UpdateTaskScheduler<TTaskKey, TCallbackArg>.ScheduledTask>
      {
