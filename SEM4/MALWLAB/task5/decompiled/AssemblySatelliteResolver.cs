  ...
  internal static class AssemblySatelliteResolver
  {
    ...
    internal static Assembly AssemblyResolve(object sender, ResolveEventArgs args)
    {
      try
      {
        ...
        string[] source = assemblyName.Name.Split('.');
        string sourceName = string.Join(".", ((IEnumerable<string>) source).Take<string>(source.Length - 1));
        string resolveFileName = assemblyName.Name + ".dll";
        List<Assembly> loadedAssemblies = new List<Assembly>((IEnumerable<Assembly>) AppDomain.CurrentDomain.GetAssemblies());
        Assembly requestingAssembly = args.RequestingAssembly;
        if (requestingAssembly != (Assembly) null)
        {
          loadedAssemblies.Remove(requestingAssembly);
          loadedAssemblies.Insert(0, requestingAssembly);
        }
        ...
        if (AssemblySatelliteResolver.ProbeViaLoadedAssemblies((ICollection<Assembly>) loadedAssemblies, assemblyName, resolveFileName, requestingAssembly, sourceName, isLog, out resolvedAssembly))
    ...
    {
      return AssemblySatelliteResolver.log.IsDebugEnabled ? (object) (Environment.NewLine + (object) new StackTrace(1, false)) : (object) string.Empty;
    }
      ...
      out Assembly resolvedAssembly)
    {
      ...
      List<Uri> loadedUris = new List<Uri>(loadedAssemblies.Count);
      foreach (Assembly loadedAssembly in (IEnumerable<Assembly>) loadedAssemblies)
      {
        ...
        if (!(requesting != (Assembly) null) || requesting.Equals((object) loadedAssembly) || name.Name.Equals(sourceName, StringComparison.OrdinalIgnoreCase))
          loadedUris.AddRange(AssemblySatelliteResolver.GetAssemblyBaseUris(loadedAssembly));
      }
      List<string> list = AssemblySatelliteResolver.ExpandCulture(resolve.CultureInfo).ToList<string>();
      Assembly resolvedAssembly1;
      if (AssemblySatelliteResolver.ProbeForAssemblySatellite((IEnumerable<Uri>) loadedUris, list, resolve, resolveFileName, isLog, first, out resolvedAssembly1))
      {
      ...
      if (isLog)
      {
        ...
        if (resolve.Name.StartsWith("SolarWinds", StringComparison.OrdinalIgnoreCase) && ((IEnumerable<string>) LocaleConfiguration.InstalledLocales).Intersect<string>((IEnumerable<string>) list, (IEqualityComparer<string>) StringComparer.OrdinalIgnoreCase).Any<string>())
...
label_19:
    ...
    private sealed class IntRef
    {
      ...
      internal IntRef(int value) => this.Value = value;

      internal int Increment() => Interlocked.Increment(ref this.Value);
    }
  }
}
