...
namespace SolarWinds.Orion.Core.BusinessLayer
{
  internal class DiscoveryNetObjectStatusManager : IDiscoveryNetObjectStatusManager
  {
    ...
    {
      get => DiscoveryNetObjectStatusManager.instance;
      internal set => DiscoveryNetObjectStatusManager.instance = value;
    }
    ...
    public void BeginOrionDatabaseChanges() => this.scheduler.BeginChanges();

    public void EndOrionDatabaseChanges() => this.scheduler.EndChanges();
    ...
    internal static void UpdateRoutine(int profileID)
    {
      ...
      using (LocaleThreadState.EnsurePrimaryLocale())
      {
        try
        {
          IEnumerable<IScheduledDiscoveryPlugin> source = DiscoveryHelper.GetOrderedDiscoveryPlugins().OfType<IScheduledDiscoveryPlugin>();
          if (source.Count<IScheduledDiscoveryPlugin>() > 0)
          {
            DiscoveryResultBase discoveryResult = DiscoveryResultManager.GetDiscoveryResult(profileID, (IList<IDiscoveryPlugin>) source.Cast<IDiscoveryPlugin>().ToList<IDiscoveryPlugin>());
            foreach (IScheduledDiscoveryPlugin ischeduledDiscoveryPlugin in source)
              ischeduledDiscoveryPlugin.UpdateImportStatuses(discoveryResult, (Action<string, double>) ((message, phaseProgress) =>
              {
