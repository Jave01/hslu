...
namespace SolarWinds.Orion.Core.BusinessLayer
{
    ...
    IServiceStateProvider
  {
    ...
    {
      get => (IServiceProvider) CoreBusinessLayerPlugin.serviceContainer;
    }
      ...
      lock (this.businessLayerServiceInstances)
      {
        ...
        else
        {
          if (!LegacyServicesSettings.Instance.NetPipeWcfEndpointEnabled)
          {
            ...
            ServiceEndpoint serviceEndpoint = this.businessLayerServiceHost.Description.Endpoints.FirstOrDefault<ServiceEndpoint>((System.Func<ServiceEndpoint, bool>) (n => n.Binding is NetNamedPipeBinding));
    ...
    private void StartSlaveEngineService(int engineId, string engineName)
    {
      lock (this.businessLayerServiceInstances)
      {
        ...
        RemoteCollectorEngineInitiator engineInitiator = new RemoteCollectorEngineInitiator(engineId, engineName, this.businessLayerService.AreInterfacesSupported, this.engineDal, (IThrottlingStatusProvider) new ThrottlingStatusProvider(), (IEngineComponent) collectorEngineComponent);
        CoreBusinessLayerServiceInstance layerServiceInstance = new CoreBusinessLayerServiceInstance(engineId, (IEngineInitiator) engineInitiator, this.businessLayerService, (ServiceHostBase) this.businessLayerServiceHost, this.serviceDirectoryClient);
        if (!this.businessLayerServiceInstances.TryAdd(engineId, layerServiceInstance))
        {
    ...
    {
      CoreBusinessLayerServiceInstance serviceInstance;
      if (this.businessLayerServiceInstances.TryGetValue(engineId, out serviceInstance))
    ...
    {
      Scheduler.Instance.Add(new ScheduledTask("UpdateEngineTable", (TimerCallback) (o => this.UpdateEngineInfoTask()), (object) null, BusinessLayerSettings.Instance.UpdateEngineTimer));
    }
    ...
    {
      Scheduler.Instance.Add(new ScheduledTask("CheckEvaluationExpiration", new TimerCallback(this.CheckEvaluationExpiration), (object) null, TimeSpan.FromHours((double) BusinessLayerSettings.Instance.EvaluationExpirationCheckIntervalHours)), true);
    }

    private static void ScheduleCheckDatabaseLimit()
    {
      Scheduler.Instance.Add(new ScheduledTask("CheckDatabaseLimit", new TimerCallback(CoreBusinessLayerPlugin.CheckDatabaseLimit), (object) "CheckDatabaseLimit", BusinessLayerSettings.Instance.CheckDatabaseLimitTimer));
    }

    private static void ScheduleCheckPollerLimit()
    {
      Scheduler.Instance.Add(new ScheduledTask("CheckPollerLimit", new TimerCallback(CoreBusinessLayerPlugin.CheckPollerLimit), (object) null, BusinessLayerSettings.Instance.PollerLimitTimer));
    }

    private static void ScheduleSavePollingCapacityInfo()
    {
      Scheduler.Instance.Add((ScheduledTask) new ScheduledTaskInExactTime("SavePollingCapacityInfo", new TimerCallback(CoreBusinessLayerPlugin.SavePollingCapacityInfo), (object) null, DateTime.Today.AddMinutes(2.0), true));
    }

    private static void ScheduleSaveElementsUsageInfo()
    {
      Scheduler.Instance.Add((ScheduledTask) new ScheduledTaskInExactTime("SaveElementsUsageInfo", new TimerCallback(CoreBusinessLayerPlugin.SaveElementsUsageInfo), (object) null, DateTime.Today.AddMinutes(1.0), true));
    }

    private static void ScheduleMaintananceExpiration()
    {
      Scheduler.Instance.Add((ScheduledTask) new ScheduledTaskInExactTime("CheckMaintenanceExpiration", new TimerCallback(CoreBusinessLayerPlugin.CheckMaintenanceExpiration), (object) null, DateTime.Today.AddSeconds(1.0), true));
    }

    private static void ScheduleCheckLicenseSaturation()
    {
      Scheduler.Instance.Add(new ScheduledTask("CheckLicenseSaturation", new TimerCallback(CoreBusinessLayerPlugin.CheckLicenseSaturation), (object) null, TimeSpan.FromMinutes((double) BusinessLayerSettings.Instance.LicenseSaturationCheckInterval)));
    }
    ...
    {
      Scheduler.Instance.Add(new ScheduledTask("LazyUpgradeTask", (TimerCallback) (o => LazyUpgradeTask.Instance.TryRunLazyUpgrade()), (object) null, TimeSpan.FromMinutes(5.0)));
    }

    private void ScheduleDeleteOldLogs()
    {
      Scheduler.Instance.Add(new ScheduledTask("DeleteOldLogs", new TimerCallback(LogHelper.DeleteOldLogs), (object) ModulesCollector.GetInstalledModules().Where<ModuleInfo>((System.Func<ModuleInfo, bool>) (x => x.RemoveOldJobResult != null)).Select<ModuleInfo, RemoveOldOnetimeJobResultsInfo>((System.Func<ModuleInfo, RemoveOldOnetimeJobResultsInfo>) (x => x.RemoveOldJobResult)).ToArray<RemoveOldOnetimeJobResultsInfo>(), BusinessLayerSettings.Instance.CheckForOldLogsTimer));
    }
    ...
    {
      Scheduler.Instance.Add(new ScheduledTask("RemoveOldOnetimeJobResults", (TimerCallback) (o =>
      {
        CoreBusinessLayerPlugin.log.DebugFormat("Clearing onetime job resuts older than {0}", (object) DiscoverySettings.OneTimeJobResultMaximalAge);
        DiscoveryResultCache.Instance.CrearOldResults(DiscoverySettings.OneTimeJobResultMaximalAge);
      }), (object) null, DiscoverySettings.OneTimeJobResultClearInetrval)
      {
    ...
    {
      Scheduler.Instance.Add(new ScheduledTask("EnhancedStatusIndication", (TimerCallback) (o => CoreBusinessLayerPlugin.EnhancedStatusIndication()), (object) null, TimeSpan.FromSeconds(30.0)));
    }
