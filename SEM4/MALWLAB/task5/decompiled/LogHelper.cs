      ...
      try
      {
        foreach (RemoveOldOnetimeJobResultsInfo onetimeJobResultsInfo in (RemoveOldOnetimeJobResultsInfo[]) state)
        {
          try
          {
            ...
            {
              if (onetimeJobResultsInfo.LogFilesPath == null)
              {
                LogHelper.log.InfoFormat("No directory specified.", Array.Empty<object>());
              }
              else
              {
                foreach (string path in onetimeJobResultsInfo.LogFilesPath)
                {
                  ...
                  else
                  {
                    ...
                    {
                      foreach (string pattern in onetimeJobResultsInfo.LogFileNamePattern)
                        LogHelper.DeleteOldFiles(directoryName, pattern, onetimeJobResultsInfo.MaxLogFileAge);
                    }
                  }
                }
              }
