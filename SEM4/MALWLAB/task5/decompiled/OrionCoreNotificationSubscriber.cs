...
namespace SolarWinds.Orion.Core.BusinessLayer
{
  public class OrionCoreNotificationSubscriber : INotificationSubscriber
  {
    ...
    {
      this._sqlHelper = sqlHelper != null ? sqlHelper : throw new ArgumentNullException(nameof (sqlHelper));
    }
      ...
      PropertyBag sourceInstanceProperties)
    {
      ...
      {
        object a;
        if (!(indicationType == IndicationHelper.GetIndicationType((IndicationType) 1)) || !((Dictionary<string, object>) sourceInstanceProperties).TryGetValue("InstanceType", out a) || !string.Equals(a as string, "Orion.Nodes", StringComparison.OrdinalIgnoreCase))
          return;
    ...
    public void Start()
    {
      Scheduler.Instance.Add(new ScheduledTask("OrionCoreIndications", new TimerCallback(this.Subscribe), (object) null, TimeSpan.FromSeconds(1.0), TimeSpan.FromMinutes(1.0)));
    }
