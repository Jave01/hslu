...
namespace SolarWinds.Orion.Core.BusinessLayer
{
  public class TechnologyPollingByPollers : ITechnologyPolling
  {
    ...
    {
      return this.pollersDAL.UpdatePollersStatus(this.pollerTypePatterns, enable, netObjectIDs).Select<PollerAssignment, int>((Func<PollerAssignment, int>) (p => p.NetObjectID)).Distinct<int>().ToArray<int>();
    }

    public int[] EnableDisableAssignment(bool enable)
    {
      return this.pollersDAL.UpdatePollersStatus(this.pollerTypePatterns, enable, (int[]) null).Select<PollerAssignment, int>((Func<PollerAssignment, int>) (p => p.NetObjectID)).Distinct<int>().ToArray<int>();
    }
