...
namespace SolarWinds.Orion.Core.BusinessLayer.Discovery
{
  internal class PartialDiscoveryResultsContainer : IDisposable
  {
    ...
    public void ClearStore() => this._persistenceStore.ClearStore();
    ...
    private OrionDiscoveryJobResult MergePartialResults(
      List<PartialDiscoveryResultsContainer.PartialResult> results)
    {
      OrionDiscoveryJobResult discoveryJobResult = (OrionDiscoveryJobResult) ((DiscoveryResultBase) ((results.Count != 0 ? results[0].Result : throw new ArgumentException("Results for merge can't be empty.", nameof (results))) ?? throw new ArgumentException("Main results for merge were not loaded.", nameof (results)))).Copy();
      foreach (PartialDiscoveryResultsContainer.PartialResult partialResult in results.Skip<PartialDiscoveryResultsContainer.PartialResult>(1))
        this.MergePluginResults(discoveryJobResult.ProfileId, ((DiscoveryResultBase) discoveryJobResult).PluginResults, ((DiscoveryResultBase) partialResult.Result).PluginResults);
      ...
      DiscoveryPluginItems<DiscoveryPluginResultBase> partialResultsToMerge)
    {
      ...
      List<DiscoveryPluginResultBase> second = new List<DiscoveryPluginResultBase>();
      foreach (DiscoveryPluginResultBase result in results)
      {
        DiscoveryPluginResultBase item = result;
        DiscoveryPluginResultBase pluginResultBase = ((IEnumerable<DiscoveryPluginResultBase>) partialResultsToMerge).Except<DiscoveryPluginResultBase>((IEnumerable<DiscoveryPluginResultBase>) second).FirstOrDefault<DiscoveryPluginResultBase>((Func<DiscoveryPluginResultBase, bool>) (x => x.GetType() == item.GetType()));
        if (pluginResultBase != null)
        {
          if (!(item is IDiscoveryPluginResultMerge pluginResultMerge))
          {
            PartialDiscoveryResultsContainer._log.WarnFormat("Plugin discovery results '{0}' do not implement IDiscoveryPluginResultMerge interface and will not be merged with other results instances.", (object) item.GetType());
          }
          else
          {
            pluginResultMerge.MergeResults(pluginResultBase, resultObjectMapping, profileId);
            second.Add(pluginResultBase);
          }
        }
      }
      foreach (DiscoveryPluginResultBase pluginResultBase in ((IEnumerable<DiscoveryPluginResultBase>) partialResultsToMerge).Except<DiscoveryPluginResultBase>((IEnumerable<DiscoveryPluginResultBase>) second))
        results.Add(pluginResultBase);
    }
    ...
    ~PartialDiscoveryResultsContainer() => this.Dispose(false);
    ...
    private class PartialResult
    {
      ...
      public PartialResult(
        ...
        DateTime expiration)
      {
        this._persistenceStore = persistenceStore != null ? persistenceStore : throw new ArgumentNullException(nameof (persistenceStore));
        this.JobId = jobId;
